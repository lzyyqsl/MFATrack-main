...
from lib.model.layers.attn_blocks import CASTKBlock
from lib.model.layers.SIFF import SIFF
from lib.model.layers.CIFF import CIFF_woGIM , GIM
#
class SCLayer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.cfm = CIFF_woGIM(dim)
        self.gim = GIM(dim)
        self.sfm = SIFF(dim, reduction=1, num_heads=4)

        self.t_fusion = nn.Sequential(
            nn.Linear(dim * 2, dim),
           nn.GELU()
        )
#s-search  t-template f-fusion 2-to v-visible i-tir

        self.ca_s2t_v2f = CASTKBlock(
        ...
        self.ca_t2s_f2i = CASTKBlock(
            dim=dim, num_heads=num_heads, mode='t2s', mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop,
            attn_drop=attn_drop, drop_path=drop_path, norm_layer=norm_layer, act_layer=act_layer
        )
        self.ca_s2t_i2f = CASTKBlock(
            dim=dim, num_heads=num_heads, mode='s2t', mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop,
            attn_drop=attn_drop, drop_path=drop_path, norm_layer=norm_layer, act_layer=act_layer
        )
        self.ca_t2s_f2v = CASTKBlock(
            dim=dim, num_heads=num_heads, mode='t2s', mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop,
            attn_drop=attn_drop, drop_path=drop_path, norm_layer=norm_layer, act_layer=act_layer
        )
        self.ca_t2t_f2v = CASTKBlock(
            dim=dim, num_heads=num_heads, mode='t2t', mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop,
            attn_drop=attn_drop, drop_path=drop_path, norm_layer=norm_layer, act_layer=act_layer
        )
        self.ca_t2t_f2i = CASTKBlock(
            dim=dim, num_heads=num_heads, mode='t2t', mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop,
            attn_drop=attn_drop, drop_path=drop_path, norm_layer=norm_layer, act_layer=act_layer
        )

    def forward(self, x_v, x_i, lens_z,global_index_template, global_index_search):
        # x_v: [B, N, C], N = 320
        # x_i: [B, N, C], N = 320
        #fused_t = torch.cat([x_v[:, :lens_z, :], x_i[:, :lens_z, :]], dim=2)
        fused_t = nn.Sequential(self.cfm([x_v[:, :lens_z, :], x_i[:, :lens_z, :]], dim=2),
                                self.gim([x_v[:, :lens_z, :], x_i[:, :lens_z, :]], dim=2),
                                self.sfm([x_v[:, :lens_z, :], x_i[:, :lens_z, :]], dim=2)
        )# CONCAT
        fused_t = self.t_fusion(fused_t)  # [B, 64, C]#fusion template image

        fused_t = self.ca_s2t_i2f(torch.cat([fused_t, x_i[:, lens_z:, :]], dim=1), global_index_template,
                                  global_index_search)[:, :lens_z, :]
