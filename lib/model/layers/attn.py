...
from lib.model.layers.rpe import generate_2d_concatenated_self_attention_relative_positional_encoding_index
...
class Attention_gui(nn.Module):
    def __init__(self, dim, mode_to, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 rpe=False, z_size=7, x_size=14):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
    ...
    def forward(self, x, mask=None, return_attention=False , mode=None, global_index_template=None, global_index_search=None):
        ...
        if self.mode_to == 's2t':  # Search to template
            q = x[:, :lens_z]  # B, lens_z, C
            k = x[:, lens_z:]  # B, lens_x, C
            v = x[:, lens_z:]  # B, lens_x, C
        elif self.mode_to == 't2s':  # Template to search
            q = x[:, lens_z:]  # B, lens_x, C
            k = x[:, :lens_z]  # B, lens_z, C
            v = x[:, :lens_z]  # B, lens_z, C
        elif self.mode_to == 't2t':  # Template to template
            q = x[:, :lens_z]  # B, lens_z, C
            k = x[:, lens_z:]  # B, lens_z, C
            v = x[:, lens_z:]  # B, lens_z, C
        elif self.mode_to == 's2s':  # Search to search
        ...
        elif self.mode_to == 't2s':
            x = torch.cat([k, x], dim=1)
        elif self.mode_to == 't2t':
            x = torch.cat([x, k], dim=1)
        elif self.mode_to == 's2s':
            x = torch.cat([x, k], dim=1)
